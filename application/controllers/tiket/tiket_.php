...
//        print_r($id_data);die();
    ...
    public function Pilih_tiketPopUp() {
        ...
        $icolumn = array('ID_TIKET_DETAIL', 'AN_TIKET', 'JNS_IDENTITAS', 'NO_IDENTITAS', 'ID_KRY', 'STATUS_TIKET', 'ID_TIKET');
...
//             die();
    ...
    public function ajax_Grid_Etiket() {
        ...
        foreach ($list as $idatatables) {
            ...
            $row[] = '<a href="#" data-toggle="modal" onclick="editData(' . $idatatables->ID_TIKET_DETAIL . ')"   data-target="#myModalUploadEtiket" class="btn btn-xs btn-primary">Upload E-Tiket</a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => $_POST['draw'],
    ...
    // $nama_dokumen = $this->input->post('nama_dokumen'); 
    // $NILAI = $this->input->post('NILAI');
    // $ID_TIKET2 = $this->input->post('ID_TIKET');
    // $dok = $this->input->post('dok');
    // print_r($_POST); die()
    // if ($_FILES["dok"]['name'] != NULL) {
    //     $new_name = time() . $_FILES["dok"]['name'];
    //     $config = array(
    //         'upload_path' => "./uploads/doc_inv/",
    //         'allowed_types' => "zip|rar",
    //         'file_name' => $new_name,
    //         'overwrite' => TRUE,
    //     );
    //     // print_r($_POST); die();
    //     $upload = $this->load->library('uploads', $config);
    //     if ($this->upload->do_upload('dok')) {
    //         $head['DOC_PATH'] = 'uploads/doc_inv/' . $new_name;
    //     } else {
    //         $this->session->set_flashdata('error', $this->upload->display_errors());
    //         redirect('tiket/tiket/home');
    //     }
    // }
    //     $data = array(
    //         'ID_TIKET' => $ID_TIKET2,
...
//        print_r($_POST);
    ...
    //  function simpan_termin(){
    //      // die('asd');
    //     // $ID_TIKET_DETAIL = $this->global_m->getIdMax('ID_TIKET_DETAIL','TBL_T_TIKET_DETAIL');
    //      $ID_TIKET_DETAIL =  explode(',', $this->input->post('sID_TIKET_DETAIL'));
    //     $ID_TIKET = trim($this->input->get('ID_TIKET'));
    //     $pay = trim($this->input->post('pay'));
    //     $payment = trim($this->input->post('payment'));
    //     $pay_term = trim($this->input->post('pay_term'));
    ...
    function tampil_payment() {
        ...
        // return $this->load-view('tiket/form_inv_tiket',$rows);
        // return $this->output->set_output(json_encode($rows));
    }
