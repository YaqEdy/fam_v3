...
//            $data['statususer'] = $this->global_m->tampil_id_desk('sec_status_user', 'statususer_id', 'statususer_desc', 'statususer_id');
    ...
    function ias_form($id) {
        ...
        // var_dump(json_encode($data['ias']));exit();
...
//            $data['statususer'] = $this->global_m->tampil_id_desk('sec_status_user', 'statususer_id', 'statususer_desc', 'statususer_id');
        ...
        $this->template->set('title', 'Cek Item');
        $this->template->load('template/template_dataTable', 'procurement/cek_barang/form_ias', $data);
    }
    ...
    public function savedata()
    {
        if (isset($_POST['repr'])) {
            ...
            $barang['ITEM_ID'] = $this->input->post('id_item');
            $barang['NAMA_BARANG'] = $this->input->post('nama_barang');
            $barang['QTY'] = $this->input->post('qty');
            $barang['TGL_TERIMA'] = DateTime::createFromFormat('d/m/Y', $this->input->post('tgl_terima'))->format('Y-m-d');
    ...
    public function ajax_GridBudgetCapex() {
        $icolumn = array('ID', 'ID_PO', 'ID_PR', 'NAMA_BARANG', 'STATUS_CEK', 'status_ke', 'TGL_PR', 'BranchID', 'BRANCH_DESC');
//        $icolumn = array('BudgetID');
        ...
        foreach ($list as $idatatables) {
            ...
            $row[] = '<a href="'.base_url().'fam_v3_/procurement/cek_barang/ias_form/'.$idatatables->ID.'" class="btn btn-primary">Cek</a>';
...
//        $icolumn = array('BudgetID');
        ...
        foreach ($list as $idatatables) {
            ...
            $row[] = $idatatables->STATUS_CEK;
            $row[] = $idatatables->ID_PO;
            $row[] = '<a href="'.base_url().'procurement/cek_barang/ias_form/'.$idatatables->ID.'" class="btn btn-primary">Cek</a>';
