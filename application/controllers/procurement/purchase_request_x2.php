...
## //------------- v3 ----------// ##
	...
	function app_requestproc() {
		...
		$status_current = $this->input->post('status');
		
		$ambil_flow = $this->db->query("
							SELECT * FROM ms_flow 
							WHERE 
								flow_id = ".$flow_id."
								and status_dari = '".$status_current."' 
								and action = 'approve'
						")->row();
        $this->db->query("UPDATE tbl_request SET status = '".$ambil_flow->status_ke."'
							WHERE RequestID = ".$RequestID."	
						");
		
        $result = array('istatus' => "a", 'iremarks' => 'Sukses.');
        // $result = array('istatus' => "a", 'iremarks' => $status_current);
        echo json_encode($result);
    }
	...
	function ajax_GridOutRequest() {
        ...
        $list = $this->datatables_custom->get_datatables('VW_PR_OUT_REQ', $icolumn, $iorder, $iwhere);
