...
class seno extends CI_Controller {
    ...
    public function index(){
        $menuId = $this->home_m->get_menu_id('reports/seno');
        ...
        $this->template->load('template/template_dataTable', 'reports/seno/index', $data);
    }

    public function list_report(){
        $icolumn = array('ID_PR','CreateDate','DivisionID','ReqTypeName','JNS_PERIODE','START_PERIODE','END_PERIODE','PSW_PO','TOTAL','TGL_BTS_KPBJ_SEWA','AGING');
        $ilike = array(
        ...
        $iorder = array();
        $list = $this->datatables_custom->get_datatables('VW_RPT_SENO', $icolumn, $iorder, $iwhere, $ilike, $igroup_by);

        $data = array();
        $no = $_POST['start'];

        foreach ($list as $idatatables) {
            if(trim($idatatables->START_PERIODE) == ''){
                $startdate = '';
            } else {
                $startdate = date('d-m-Y', strtotime($idatatables->START_PERIODE));
            }

            if(trim($idatatables->END_PERIODE) == ''){
                $enddate = '';
            } else {
                $enddate = date('d-m-Y', strtotime($idatatables->END_PERIODE));
            }
            ...
            $row[] = $idatatables->JNS_PERIODE;
            $row[] = $startdate;
            $row[] = $enddate;
            $row[] = $idatatables->PSW_PO;
            $row[] = number_format($idatatables->TOTAL,2);
            $row[] = date('d-m-Y', strtotime($idatatables->TGL_BTS_KPBJ_SEWA));
            $row[] = $idatatables->AGING;
            $data[] = $row;
        }
    ...
    public function downloadReport() {
        if ($this->auth->is_logged_in() == false) {
            ...
            $iorder = "ID_PR";
            $data ['list'] = $this->datatables_custom->get_datatables_print('VW_RPT_SENO', $icolumn, $iorder, $iwhere, $ilike, $igroup_by);
            // print_r($data ['list']);die();
            $this->load->view('reports/seno/seno_excel', $data);
        }
    }
    ...
    public function downloadReportPDF() {
        if ($this->auth->is_logged_in() == false) {
            ...
            $iorder = "ID_PR";
            $data ['list'] = $this->datatables_custom->get_datatables_print('VW_RPT_SENO', $icolumn, $iorder, $iwhere, $ilike, $igroup_by);
            $this->load->view('reports/seno/seno_pdf', $data);
        }
    }
}
?>