...
//        $icolumn = array('HpsID');
    ...
    public function ajax_GridPopUpUser() {
        ...
        foreach ($list->profile[0]->data as $idatatables) {
            ...
            $row[] = $idatatables->profile_posisi;
    ...
    function get_user_fam() {
        return $this->api_m->get_user_fam();
    }
...
//     }
    ...
    function tampil_data() {
        ...
        // print_r($idsm); die();
        $sql = "select nik, user_name, name, DivisionID, status, user_groupid, idsdm, ZoneID, BranchID, user_id
                FROM [user]
    ...
    function ubah() {
        ...
        // $table = "user";
        // $id_kolom = "user_id = '".$user_id."'";
        // print_r($id_kolom);die();
        $model = $this->global_m->ubah('user', $datax, 'user_id', $nik);
        if ($model) {
            $array = array(
                'act' => 1,
                'tipePesan' => 'success',
