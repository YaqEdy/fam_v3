...
class tiket_m extends CI_Model {
    ...
    function getAllTiket($ID_TIKET_DETAIL) {
        ...
        $division = $this->db2->query("SELECT AKOMODASI, AN_TIKET, NOTE, JNS_IDENTITAS, NO_IDENTITAS, ASAL_BERANGKAT, STATUS_TIKET,  TUJUAN_BERANGKAT, KATEGORI, TGL_BERANGKAT, TGL_PULANG, ID_TIKET_DETAIL
                        FROM  VW_TIKET WHERE ID_TIKET IN (".$ID_TIKET_DETAIL.") ");
        // print_r($division); die();
        return $division->result();
    }

    public function simpanubah($datax,$data,$ID_TIKET){
		$this->db->trans_begin();
