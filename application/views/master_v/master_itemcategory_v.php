...
<script>
        ...
        bootbox.confirm("Apakah anda yakin meng-aktifkan data " + idata[3] + "?", function (o) {
            if (o == true) {
                $.ajax({
                    ...
                    cache: false,
                    dataType: "JSON",
                    url: "<?php echo base_url("/master/master_itemcategory/ajax_UpdateStatusCategory"); ?>", // json datasource
    ...
    function clickUpdate() {
        ...
        if (ItemCategoryCode == "") {
            bootbox.alert({
                message: "Required Item Category Code",
                backdrop: true
            });
        ...
        //     });
            ...
            if (status == "add") {
                var message = 'Apakah anda yakin ingin menambahkan data item category?';
            } else {
                var message = 'Apakah anda yakin ingin mengubah data item category?';
            }
            bootbox.confirm(message, function (o) {
                if (o == true) {
                    $.ajax({
                        ...
                        cache: false,
                        dataType: "JSON",
                        url: "<?php echo base_url("/master/master_itemcategory/ajax_UpdateCategory"); ?>", // json datasource
                        data: {sTbl: i_clsUpdate},
                        success: function (e) {
                            console.log(e);
                            if (e.msgType == true) {
...
//                // set the initial value
            ...
            "serverSide": true,
            "ajax": {
                url: "<?php echo base_url("/master/master_itemcategory/get_server_side"); ?>", // json datasource
