        ...
        bootbox.confirm("Apakah anda yakin meng-aktifkan data " + idata[3] + "?", function (o) {
            if (o == true) {
                $.ajax({
                    ...
                    cache: false,
                    dataType: "JSON",
                    url: "<?php echo base_url("/master/master_zonasi/ajax_UpdateStatusCategory"); ?>", // json datasource
...
//        console.log(i_clsUpdate)
        ...
        var zona_name = $("#txtZoneName").val();
        if (zona_name == "") {
            bootbox.alert({
                message: "Required Zone Name",
                backdrop: true
            });
        } else {
            if (status == "add") {
                var message = 'Apakah anda yakin ingin menambahkan data zonasi?';
            } else {
            ...
            bootbox.confirm(message, function (o) {
                if (o == true) {
                    $.ajax({
                        ...
                        cache: false,
                        dataType: "JSON",
                        url: "<?php echo base_url("/master/master_zonasi/ajax_UpdateCategory"); ?>", // json datasource
                        data: {sTbl: i_clsUpdate},
                        success: function (e) {
                            console.log(e);
                            if (e.msgType == true) {
...
//                // set the initial value
            ...
            "serverSide": true,
            "ajax": {
                url: "<?php echo base_url("/master/master_zonasi/get_server_side"); ?>", // json datasource
